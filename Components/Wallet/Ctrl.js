...
$scope.SaveWallet = function(){
	...
	let l = $rootScope.M4M.Spends.data.filter(c => !$scope.Wallet.some(o => o.id == c.wallet));
	if(l.length > 0) {
		$rootScope.AddNotifis($translate.instant('notifi.fs_delete_duplicate', ({...firestore, dup: l[0].wallet})), "warning");
		return false;
	}
